...
BRM.3Full <- multicon::composite(BRM.3r,
  rel = TRUE, 
  nomiss = 0.6)
...
BRM.3Three <- multicon::composite(BRM.3rawThree,
  rel = TRUE, 
  nomiss = 0.6)
...
NRM.3Full <- multicon::composite(NRM.3raw,
  rel = TRUE, 
  nomiss = 0.6)
...
RM_S3Full <- dplyr::coalesce(BRM.3Full, NRM.3Full)
RM_S3Three <- dplyr::coalesce(BRM.3Three, NRM.3Three)
...
```{r}
vars <- data.frame(IN_S3, RM_S3Full, RM_S3Three, OK_S3, Study3rawdataset$Condition_S3)

write.csv(vars, "Study3_SPSS.csv")
```


```{r Serial Mediation with Full race mini}
X <- Study3rawdataset$Condition_S3
M1 <- IN_S3
M2 <- RM_S3Full
Y <- OK_S3
Data <- data.frame(X=Study3rawdataset$Condition_S3, Y=OK_S3, M1=IN_S3, M2=RM_S3Full)
...
M2 <- RM_S3Three
Y <- OK_S3
Data <- data.frame(X=Study3rawdataset$Condition_S3, Y=OK_S3, M1=IN_S3, M2=RM_S3Three)
