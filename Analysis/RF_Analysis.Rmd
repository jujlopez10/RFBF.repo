...
#Power test
power.t.test(n = 77, delta = (3.32 - 3.02), sig.level = .05, power = NULL, alternative = "two.sided", type = "one.sample")
...
Study1Arawdataset <- mutate(Study1Arawdataset, Condition = case_when(RFfirst_1 == 1 | RFfirst_2 == 1 ~ "1", BFfirst_1 == 1 | BFfirst_2 == 1 ~ "0"))
```
...
```{r}
Study1Arawdataset <- Study1Arawdataset %>% mutate(OK = coalesce(RF1aOK, BF1aOK))

lm <- lm(OK ~ 1 + Condition, data = Study1Arawdataset)
summary(lm)
```
...
#Power test
power.t.test(n = 102, delta = (3.14 - 2.8), sig.level = .05, power = NULL, alternative = "two.sided", type = "one.sample")
```
...
Study1Brawdataset <- mutate(Study1Brawdataset, Condition = case_when(RFfirst_1 == 1 | RFfirst_2 == 1 ~ "1", BFfirst_1 == 1 | BFfirst_2 == 1 ~ "0"))
```
...
```{r}
Study1Brawdataset <- Study1Brawdataset %>% mutate(OK = coalesce(RF1bOK, BF1bOK))

lm <- lm(OK ~ 1 + Condition, data = Study1Brawdataset)
summary(lm)
```
...
#Power test
power.t.test(n = 74, delta = (2.84 - 2.33), sig.level = .05, power = NULL, alternative = "two.sided", type = "two.sample")
```
...
RM_S2Full <- dplyr::coalesce(BRM.2, NRM.2)
RM_S2Three <- dplyr::coalesce(BRM.2_3, NRM.2_3)

# t.test(NRM.2_3, BRM.2_3, paired = FALSE, var.equal = TRUE)

```

```{r Mediation with full RM }
X <- Study2rawdataset$Condition_S2
M <- RM_S2Full
Y <- OK_S2
Data <- data.frame(X=Study2rawdataset$Condition_S2, Y=OK_S2, M=RM_S2Full)
...
```{r Mediation with three item RM }
X <- Study2rawdataset$Condition_S2
M <- RM_S2Three
Y <- OK_S2
Data <- data.frame(X=Study2rawdataset$Condition_S2, Y=OK_S2, M=RM_S2Three)
...
# Con <- Study2rawdataset$Condition_S2
# RM <- RM_S2Three
# OK <- OK_S2
# vars <- data.frame(Con, RM, OK)

# write.csv(vars, "Study2_SPSS.csv")
```
...
```{r Power Test}
#Power test
power.t.test(n = 648, delta = (3.52 - 2.66), sig.level = .05, power = NULL, alternative = "two.sided", type = "two.sample")
```
...
Creating Acceptability Variable combining Redface and Blackfacef
```{r OK}
...
Study3rawdataset <- mutate(Study3rawdataset, BFOK_S3 =
...
Study3rawdataset <- mutate(Study3rawdataset, RFOK_S3 = 
  multicon::composite(RFS3.full,
                      rel = TRUE,
                      nomiss = 0.6))

Study3rawdataset <- mutate(Study3rawdataset, OK_S3 = dplyr::coalesce(BFOK_S3, RFOK_S3))
...
Study3rawdataset <- mutate(Study3rawdataset, BRM.3Three = multicon::composite(BRM.3rawThree,
  rel = TRUE, 
  nomiss = 0.6))
...
NRM.3Three <- multicon::composite(NRM.3rawThree,
  rel = TRUE, 
  nomiss = 0.6)
...
Study3rawdataset <- mutate(Study3rawdataset, RM_S3Three = dplyr::coalesce(BRM.3Three, NRM.3Three))
...
t.test(NRM.3Three, BRM.3Three, paired = FALSE, var.equal = TRUE)
...
Study3rawdataset <- mutate(Study3rawdataset, BIN_S3 = multicon::composite(BIN.3r,
  rel = TRUE, 
  nomiss = 0.6))

```

```{r Native Invisibility}
...
Study3rawdataset <- mutate(Study3rawdataset, NIN_S3 = multicon::composite(NIN.3r,
  rel = TRUE, 
  nomiss = 0.6))

```

```{r Overall Invisibility}

Study3rawdataset <- mutate(Study3rawdataset, IN_S3 = dplyr::coalesce(BIN_S3, NIN_S3))


```

```{r Invisibility Ttest}

t.test(Study3rawdataset$NIN_S3, Study3rawdataset$BIN_S3, paired = FALSE, var.equal = TRUE)

```

```{r Creating mplus file with comps}


OK <- Study3rawdataset$OK_S3
RM <- Study3rawdataset$RM_S3Three
IN <- Study3rawdataset$IN_S3
Target <- Study3rawdataset$Condition_S3

vars <- data.frame(OK, RM, IN, Target)

# write.csv(vars, "C:/Users/Julisa/Documents/RFBF/RFBF.repo/Data/Study3MplusComposite.csv")

```


```{r Option A - Serial Mediation Model}
...
"
  #Regressions
  IN ~ a1 * Condition
  RM ~ m * IN + a2 * Condition
  OK ~ b2 * RM + b1 * IN + c * Condition
...
```{r Option B Serial Mediation Model}
library(MeMoBootR)

OK <- data$OK_S3
Condition <- data$Study3rawdataset.Condition_S3
IN <- data$IN_S3
RM <- data$RM_S3Three
...
vars <- data.frame(OK, Condition, IN, RM)
...
```{r} 
# Prepping data for nalysis with 3 item invisibility 
...
vars <- data.frame(RFOK, BFOK, BRM, NRM, BIN, NIN)

# write.csv(vars, "C:/Users/Julisa/Documents/RFBF/RFBF.repo/Data/Study4_SPSS_2.csv")
```


```{r Plots for 3 item IN}
# Looking at plots with 3 item invisibility 


RFOK <- Study4rawdataset$RFS4.comp
BFOK <- Study4rawdataset$BFS4.comp
BRM <- BRM_S4Three
NRM <- NRM_S4Three
BIN <- BIN_S4
NIN <- NIN_S4
OKdiff <- (Study4rawdataset$RFS4.comp - Study4rawdataset$BFS4.comp)
RMdiff <- (NRM_S4Three - BRM_S4Three)
INdiff <- (NIN_S4 - BIN_S4)

data <- data.frame(BFOK, RFOK, BRM, NRM, NIN, BIN) # put all variables in easy access data frame

data <- data[-c(1:6, 780), ] # deleted empty cases

# write.csv(data, "Study4_SPSS.csv")

df2 <- data.frame(OK = c(data[,"RFS4.comp"], data[,"BFS4.comp"])) # stacking RFOK + BFOK = OK
df3 <- data.frame(RM = c(data[,"NRM"], data[,"BRM"])) # stacking NRM + BRM = RM
df4 <- data.frame(IN = c(data[,"NIN"], data[,"BIN"])) # stacking NIN + BIN = IN

                  
mod_data <- data.frame(df2, df3, df4) # combining all moderation variables in easy access data frame

mod_data$ID <- seq.int(nrow(mod_data)) # creating row ID

mod_data <- mutate(mod_data, Target = ifelse(ID == c(1:773), 1, -1)) # Creating target variable

mod_data <- mutate(mod_data, BlkTarget = ifelse(ID == c(1:773), 1, 0)) # Creating Black target variable

mod_data <- mutate(mod_data, NtvTarget = ifelse(ID == c(1:773), 0, 1)) # Creating Native target variable


overall_plot <- 
  ggplot(
  mod_data, aes(x=IN, y=OK, color=as.factor(Target))) + #Information for graph
  geom_point() + # Type of plot
  xlab("Invisibility of Target Groups") + # labeling x axis 
  ylab("Acceptability of Racialized Represenations") + # labeling y axis
  scale_color_manual(
  values = c("#000000", "#FF0000"), name = "Target Group", labels = c("Black People", "Native People")) #coloring and labels for legend

overall_plot

native_plot <- ggplot(data, aes(x=NIN, y=RFS4.comp)) + geom_point(color = "red") + xlab("Invisibility of Native People") + ylab("Acceptability of Redface")

native_plot

black_plot <- ggplot(data, aes(x=BIN, y=BFS4.comp)) + geom_point() + xlab("Invisibility of Black People") + ylab("Acceptability of Blackface")
...
```{r Model with Full RM}
vars <- data.frame(RFS4.comp, BFS4.comp, BRM_S4Full, NRM_S4Full, NIN_S4, BIN_S4)

# Racism Mini model
mediation_model.RM <- mdt_within_wide(vars, DV_A = RFS4.comp, DV_B = BFS4.comp, M_A = NRM_S4Full, M_B = BRM_S4Full)

mediation_model.RM$paths

add_index(mediation_model.RM, times = 5000, level = 0.05)

# Invisibility Model
mediation_model.IN <- mdt_within_wide(vars, DV_A = RFS4.comp, DV_B = BFS4.comp, M_A = NIN_S4, M_B = BIN_S4)

mediation_model.IN$paths

add_index(mediation_model.IN, times = 5000, level = 0.05)
```


```{r Model with 3 item RM}
vars <- data.frame(RFS4.comp, BFS4.comp, BRM_S4Three, NRM_S4Three, NIN_S4, BIN_S4)

# Racism Mini model
mediation_model.RM <- mdt_within_wide(vars, DV_A = RFS4.comp, DV_B = BFS4.comp, M_A = NRM_S4Three, M_B = BRM_S4Three)
