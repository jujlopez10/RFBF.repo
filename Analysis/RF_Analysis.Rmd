...
Study1Arawdataset<- Study1Arawdataset[-c(1:9), ]  
...
Study1Brawdataset<- Study1Brawdataset[-c(1), ]  
...
RawdataStudy2 <- read.csv("/Users/jujlopez/Documents/Redface/Data/Study2.csv", header = T, sep = ",", na=-99, stringsAsFactors = FALSE)

# RawdataStudy2 <- read.csv("C:/Users/Julisa/Documents/RFBF/Data/Study2.csv", header = T, sep = ",", na=-99, stringsAsFactors = FALSE)

RawdataStudy2<- RawdataStudy2[-c(1:2), ]  # Getting rid of unneccesary rows

RawdataStudy2 <- RawdataStudy2 %>% retype()

RawdataStudy2 <- mutate(RawdataStudy2, Condition = ifelse(is.na(RawdataStudy2$BFPaint1), 1, 0)) # creating condition variable

```

```{r BF OK}
BFS2.raw <- RawdataStudy2 %>% filter(Condition == 0) %>% dplyr::select(
  BFPaint1_1, BFPaint1_2, BFPaint1_3, BFPaint1_4,
  BFPaintWom1_1, BFPaintWom1_2, BFPaintWom1_3, BFPaintWom1_4,
  BFGroup1_1, BFGroup1_2, BFGroup1_3, BFGroup1_4,
  BFPaintMan2_1, BFPaintMan2_2, BFPaintMan2_3, BFPaintMan2_4,
...
```{r RF OK}
RFS2.raw <- RawdataStudy2 %>% filter(Condition == 1) %>% dplyr::select(
  RFPaintGroup2_1, RFPaintGroup2_2, RFPaintGroup2_3, RFPaintGroup2_4, 
  RFPaintWom1_1, RFPaintWom1_2, RFPaintWom1_3, RFPaintWom1_4, 
  RFPaintGroup1_1, RFPaintGroup1_2, RFPaintGroup1_3, RFPaintGroup1_4,
  RFPaintMan2_1, RFPaintMan2_2, RFPaintMan2_3, RFPaintMan2_4, 
...
```{r OK}
BFS2.fullraw <- RawdataStudy2 %>% dplyr::select(
  BFPaint1_1, BFPaint1_2, BFPaint1_3, BFPaint1_4,
  BFPaintWom1_1, BFPaintWom1_2, BFPaintWom1_3, BFPaintWom1_4,
  BFGroup1_1, BFGroup1_2, BFGroup1_3, BFGroup1_4,
  BFPaintMan2_1, BFPaintMan2_2, BFPaintMan2_3, BFPaintMan2_4,
...
RFS2.fullraw <- RawdataStudy2 %>% dplyr::select(
  RFPaintGroup2_1, RFPaintGroup2_2, RFPaintGroup2_3, RFPaintGroup2_4, 
  RFPaintWom1_1, RFPaintWom1_2, RFPaintWom1_3, RFPaintWom1_4, 
  RFPaintGroup1_1, RFPaintGroup1_2, RFPaintGroup1_3, RFPaintGroup1_4,
  RFPaintMan2_1, RFPaintMan2_2, RFPaintMan2_3, RFPaintMan2_4, 
...
RawdataStudy2 <- RawdataStudy2 %>% mutate(RFS2.full.comp = rowMeans(RFS2.full, na.rm = TRUE))
RawdataStudy2 <- RawdataStudy2 %>% mutate(BFS2.full.comp = rowMeans(BFS2.full, na.rm = TRUE))

RawdataStudy2$RFS2.full.comp[is.nan(RawdataStudy2$RFS2.full.comp)] <- NA

RawdataStudy2$BFS2.full.comp[is.nan(RawdataStudy2$BFS2.full.comp)] <- NA

RawdataStudy2 <- RawdataStudy2 %>% mutate_at(vars(RFS2.full.comp:BFS2.full.comp), as.character) %>% tidyr::unite(OK, RFS2.full.comp:BFS2.full.comp, na.rm = TRUE, remove = FALSE)

RawdataStudy2$OK <- as.numeric(RawdataStudy2$OK)
...
```{r Black RM}
BRM.raw <- RawdataStudy2 %>% dplyr::select(
...
```{r Native RM}
NRM.raw <- RawdataStudy2 %>% dplyr::select(
...
```{r}
RawdataStudy2 <- RawdataStudy2 %>% mutate(NRM.comp = multicon::composite(NRM.r, rel = TRUE, nomiss = 0.6))
RawdataStudy2 <- RawdataStudy2 %>% mutate(BRM.comp = multicon::composite(BRM.r, rel = TRUE, nomiss = 0.6))

RawdataStudy2$NRM.comp[is.nan(RawdataStudy2$NRM.comp)] <- NA

RawdataStudy2$BRM.comp[is.nan(RawdataStudy2$BRM.comp)] <- NA

RawdataStudy2 <- RawdataStudy2 %>% mutate_at(vars(NRM.comp:BRM.comp), as.character) %>% tidyr::unite(RM, NRM.comp:BRM.comp, na.rm = TRUE, remove = FALSE)

RawdataStudy2$RM <- as.numeric(RawdataStudy2$RM)
```

```{r Mediation}
X <- RawdataStudy2$Condition
M <- RawdataStudy2$RM
Y <- RawdataStudy2$OK
